<template>
  <div class="slide-inner">
    ...
    >
      <div v-for="(item, index) in imgList" :key="item.id" class="slide-img-item">
...
export default {
  ...
  computed: {
    ...
    slideTransitionDuration() {
      return this.isPreventTransition ? "0ms" : "500ms";
    },
    zoomHorizontalMovable() {
      return (
        document.getElementById(`slide-img-${this.currentIndex}`).offsetWidth * this.pinchZoomScale >
        this.$refs["slide-contents"].offsetWidth
      );
    },
    zoomVerticalMovable() {
      return (
        document.getElementById(`slide-img-${this.currentIndex}`).offsetHeight * this.pinchZoomScale >
    ...
    moveZoomPosition(e) {
      ...
      if (this.zoomVerticalMovable) {
        if (this.zoomVerticalDistance >= Math.abs(this.currentZoomPositionY + movedDragDistanceY)) {
          ...
          movedDragPositionY =
            this.currentZoomPositionY + movedDragDistanceY > 0 ? this.zoomVerticalDistance : -this.zoomVerticalDistance;
        }
      } else {
        movedDragPositionY = 0;
      }
      document.getElementById(`slide-img-wrap-${this.currentIndex}`).style.transform = `translate(
